...
unsigned long long main_cpu_clk = 0;

extern void USBSID_WriteOperation(cRSID_C64instance *C64, unsigned short address, unsigned char data);
...
unsigned char cRSID_emulateCPU (void) { //the CPU emulation for SID/PRG playback (ToDo: CIA/VIC-IRQ/NMI/RESET vectors, BCD-mode)
 ...
 inline unsigned char rd (register unsigned short address) {
  ...
  // USBSID_ReadOperation(C64, address);
  if (C64->RealSIDmode) {
   if ( C64->RAMbank[1] & 3 ) {
    if (address==0xDC0D) { cRSID_acknowledgeCIAIRQ( &C64->CIA[0] ); }
    else if (address==0xDD0D) { cRSID_acknowledgeCIAIRQ( &C64->CIA[1] ); }
   }
  }
  return value;
 }
  ...
  *cRSID_getMemWritePtr(address)=data;
  USBSID_WriteOperation(C64, address, data);
  ...
  *cRSID_getMemWritePtr(address)=data;
  ...
  if ( C64->RAMbank[1] & 3 ) {
   if (C64->RealSIDmode) {
    if (address==0xDC0D) cRSID_writeCIAIRQmask( &C64->CIA[0], data );
    else if (address==0xDD0D) cRSID_writeCIAIRQmask( &C64->CIA[1], data );
 ...
 if(IR&1) {  //nybble2:  1/5/9/D:accu.instructions, 3/7/B/F:illegal opcodes
  ...
  switch ( (IR & 0xE0) >> 5 ) { //value-forming to cause gapless case-values and faster jump-table creation from switch-case
   ...
   case 3: if ( (IR&0x1F) != 0xB ) { //RRA (ROR+ADC) / ADC
            ...
            T=A; A += rd(Addr)+(ST&C);
            if ((ST & D) && (A&0xF)>9) { A+=0x10; A&=0xF0; } //BCD?
  ...
  if ( (IR&0x1F) == 0x10 ) { //BPL/BMI/BVC/BVS/BCC/BCS/BNE/BEQ  relative branch
   ...
   switch ( (IR & 0xE0) >> 5 ) {
    ...
    case 2: if (IR & 0xF) { //JMP / NOP-abs,x/zp/zp,x
             ...
             if (C64->Returned && SP>=0xFF) { ++PC; storeReg(); main_cpu_clk += Cycles; return 0xFE; }
            }
            break;
    ...
    case 3: if (IR & 0xF) { //JMP() (indirect) / NOP-abs,x/zp/zp,x
             ...
             if (SP>=0xFF) {storeReg(); C64->Returned=1; main_cpu_clk += Cycles; return 0xFF;} //Init returns, provide idle-time between IRQs
