...
An optional Volume daughterboard is available for independent left/right channel volume control. It is _not_ tested with v1.0 PCB's and requires an extra ground mod - see the `USBSID-Volume-daughterboard` manual for details.
