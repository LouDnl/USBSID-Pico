...
Socket voltage control on v1.5+ is more than just the automatic 9v/12v selection above - power to both SID sockets can also be fully switched off and back on again, independent of SID type, through the configtool or firmware settings.
...
The USBSID-Pico v1.5 PCB has the option for you to solder your Pico directly to the PCB using the Pico's castelated edges. +
...
_(markings are on the bottom of the PCB)_ +
*NOTE:* The `USE_PIO_UART` firmware build option (PIO UART on GPIO 26/27) is not available on v1.5+ boards, those GPIO pins are used for socket voltage control instead.


== Audio switch
Same digital audio switch IC as on v1.3, carried forward unchanged. Switching from mono (both SID chips play over left and right) to Stereo (SID1 over the left channel and SID2 over the right channel) and back is handled by the firmware through settings or automatically by supported players. +
The jumperless v1.5 design has no physical `SW` pin for manual switching - this is configured entirely through the firmware/configtool.


[%always]
<<<

== Volume daughterboard
An optional Volume daughterboard is available for independent left/right channel volume control. It is plug and play compatible with v1.5+ boards - see the `USBSID-Volume-daughterboard` manual for details.
