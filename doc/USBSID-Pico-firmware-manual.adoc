...
=== Firmware naming
`usbsidA-BC.uf2` examples: `usbsidpico-v1.0-rgb.uf2`, `usbsidpico2_w-v1.3.uf2` or `usbsidpico2-v1.5-rgb_em`. +
A: The pico board type e.g. `pico`, `pico2`, `pico_2` or `pico2_w` +
B: The PCB version e.g. `v1.0`, `v1.3` or `v1.5` +
C: A combination of optional features e.g. `-rgb`, `_em` or `_net`
...
=== Firmwares with embedded USBSID-Player and Cynthcart
Firmwares ending with `_em` have an embedded USBSID-Player that can play PSID and RSID tunes and PSID64 PRG/P00 files, and can also start Cynthcart via CC commands. Both now run through the same embedded player core. +
For example `usbsidpico-v1.0_em.uf2`, `usbsidpico2-v1.3-rgb_em.uf2` or `usbsidpico2_w-v1.5_em_net.uf2` +
This USBSID-Player is still in active development and some tunes might not work or play at all. +
The embedded player needs a large chunk of RAM (over 80% on rp2040 boards, around 40% on rp2350 boards) and an overclocked CPU, but is now built for every Pico type, rp2040 included. On `pico2_w` it is only ever offered bundled with Network SID Device support (`_em_net`), since that board has RAM to spare for both together. +
**WARNING!** Whenever the embedded player or Cynthcart is used the Pico MCU is overclocked to 250MHz, regardless of Pico type. Passive cooling _might/could_ be needed. +
See the `USBSID-Player-manual` and `USBSID-Pico-Cynthcart-manual` files for more information.
...
The tool is available here https://github.com/LouDnl/USBSID-Configtool[USBSID-Configtool] in the releases section
...
Asid Enabled: `True` +
Midi Enabled: `True` +
Net Enabled: `False` (`_w` boards only) +
NSD Enabled: `False` (`_w` boards only) +
FMOpl Enabled: `False` +
FMOpl SID no: `0`
...
==== Flip Socket One and Socket Two
...
==== Mix socket addresses (Quad SID only)
...
==== Preset silent auto detection
When enabled, the board silently re-runs chip/SID auto detection before applying a preset. +
Default is `False` on PCB revision v1.0~v1.3 and `True` on PCB revision v1.5+. +
Available options are: +
`False`, `True`
...
=== Midi Enabled
Disable or enable MIDI input handling (note/CC/voice control of the SID(s)). +
Available options are: +
`False`, `True`

=== WiFi Enabled
_`pico_w`/`pico2_w` boards firmware with _net only_ +
_This config setting is intended for future use, no CLI/GUI/Web configtool support yet_ +
Enables the board's WiFi station connection. Requires an SSID and password to have been provisioned first. +
Available options are: +
`False`, `True`
...
_`pico_w`/`pico2_w` boards firmware with _net only_ +
_This config setting is intended for future use, no CLI/GUI/Web configtool support yet_ +
Enables the Network SID Device (NSD) server: a TCP listener (default port `6581`) plus a UDP discovery responder, letting NSD clients such as ACID 64, JSidplay2 and SIDPlay find and drive the board over the LAN exactly like a native NSD server. Only one NSD session (WiFi or Bluetooth) can be active at a time; USB always keeps priority over the SID bus. +
Available options are: +
`False`, `True`

=== Bluetooth NSD Enabled
_`pico_w`/`pico2_w` boards firmware with _net only_ +
_This config setting is intended for future use, no CLI/GUI/Web configtool support yet_ +
Enables the Bluetooth Classic SPP transport for the same Network SID Device protocol as `NSD Enabled` above. No NSD client speaks Bluetooth SPP directly, so this needs a small host-side RFCOMM-to-TCP bridge (see `examples/nsd_spp_bridge.py`) to be usable with existing players. +
Available options are: +
`False`, `True`
